...
public UnitOfWork(ApplicationDbContext db, IDocumentRepository d, IUserRepository u) { ... }
```
...
- **LINQ projection lambdas** where the body references the variable once and the type is obvious from context:
  ```csharp
  db.Documents.Where(document => document.UserId == userId).Select(document => document.Id)
  ```
