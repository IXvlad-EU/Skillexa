---
description: "Best practices for building Next.js (App Router) apps with modern caching, tooling, and server/client boundaries."
...
- **TypeScript:** Use TypeScript for all code. Enable `strict` mode in `tsconfig.json`. Set `"target": "ES2022"` (or later) to enable modern type-checking (e.g., `Array.at()`, `structuredClone`, `Error.cause`).
- **ESLint & Prettier:** Enforce code style and linting. Use the official Next.js ESLint config. In Next.js 16, prefer running ESLint via the ESLint CLI (not `next lint`). Use the **flat config** format (`eslint.config.mjs`) with `eslint-config-next/core-web-vitals` and `eslint-config-next/typescript`.
